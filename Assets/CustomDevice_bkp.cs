...
public class CustomDevice : InputDevice, IInputUpdateCallbackReceiver
{
    ...
    public void OnUpdate()
    {
        if (_button == null)
        {
            _button = Transform.FindObjectOfType<VRPN_Button>();
            _button.OnButtonDown.AddListener(_ => { _buttonPressed = true; Debug.Log("Button Down"); });
            _button.OnButtonUp.AddListener(_ => { _buttonPressed = false; Debug.Log("Button Up"); });
        ...
        //q = _tracker.Host.GetRotation("Tracker0", 0);
